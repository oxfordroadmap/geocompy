...
-   *Learning Geospatial Analysis with Python*[^book_1] 和 *Geoprocessing with Python*[^book_2] 是本領域的書籍，專注於使用 Python 的低階介面處理空間資料，特別是透過 **osgeo** 套件中的 **gdal**、**gdalnumeric** 和 **ogr** 等模組。這種作法通常需要撰寫較多的程式碼。考量到 **geopandas** 和 **rasterio** 等新興套件的發展，我們認為本書所採用的方法更符合 Python 語言風格（'Pythonic'）且具備前瞻性。
-   *Introduction to Python for Geographic Data Analysis*[^book_3]（尚在撰寫中）旨在提供「Python 地理資訊系統（GIS）」的基礎入門，其中涵蓋 Python 基礎語法、Python 在 GIS 中的應用，以及實務案例分析。相較於本書——同樣為開放原始碼教材，並托管於 pythongis.org——*Geocomputation with Python* 的範疇較為聚焦（例如未涵蓋空間網路分析），但更強調柵格資料的處理與柵格向量資料的互操作性。
-   *Geographic Data Science with Python*[^book_4] 為雄心勃勃的專案，其章節專注於進階主題，例如第 4 章即深入探討空間權重（Spatial Weights），內容複雜且進入時程較早。
-   *Python for Geospatial Data Analysis*[^book_5] 涵蓋使用 Python 處理地理空間資料的多種方式，包括專有與開源 GIS 軟體的自動化操作，以及獨立的開源 Python 套件（亦是本書重點剖析的領域）。相較之下，Geocompy 更簡潔、容易上手且具入門性，並以同等重視的角度探討柵格與向量資料。
...
另一項本書的獨特特色是其屬於更廣泛的社群體系。
*Geocomputation with Python* 是 *Geocomputation with R*[^geocompr][@lovelace_geocomputation_2019] 的姊妹計畫，後者是一本以 R 語言進行地理資料分析、視覺化與建模的著作，擁有超過 60 位貢獻者及活躍社群，尤其是在其相關的 Discord 群組中[^geocompr_discord]。
與充滿活力的 'R-spatial' 社群，以及如 GeoRust 和 JuliaGeo 等其他開源社群的連結，創造出跨生態系統的交流與互利合作機會。
